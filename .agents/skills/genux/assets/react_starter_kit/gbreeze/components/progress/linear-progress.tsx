...
function parseCssPx(value: string, fallback = 4): number {
...
function WavyLinearProgress({
    ...
    if (isDeterminate) {
      ...
      if (value === 100) {
        if (rootRef.current) {
          ...
          const strokeWidth = parseCssPx(
            style.getPropertyValue('stroke-width'),
            4,
          );
          const gapSize = parseCssPx(style.getPropertyValue('min-height'), 4);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (!isDeterminate) {
        return; // Handled purely via CSS when indeterminate in GM3
      }
      ...
      const style = window.getComputedStyle(rootRef.current);
      const strokeWidth = parseCssPx(style.getPropertyValue('stroke-width'), 4);
      const gapSize = parseCssPx(style.getPropertyValue('min-height'), 4);
      const effectiveLength = length - 2 * strokeWidth - gapSize;
      const center = rect.height / 2;
...
/** Material 3 LinearProgress component supporting determinate, indeterminate, and wavy styles. */
