...
authoritative JSON output schema, consult the visual assessor framework).

**CRITICAL PRINCIPLE - MATERIAL ADHERENCE IS IRRELEVANT:** The sole purpose of
this detection phase is component identification. Whether a detected component
adheres to Material Design guidelines is completely irrelevant. Do not filter
out or misclassify components based on whether they follow Material Design
rules.
...
1.  List the contents of the guidelines directory.
2.  For every component type you identify or suspect is present on the screen,
    you MUST read its corresponding detailed guideline/rubric definitions (e.g.,
    if you suspect a FAB, verify against its specific visual heuristics) before adding it to the final output.
3.  Do NOT rely on default model knowledge; the heuristics in these guidelines
    and rubrics are authoritative for this audit.
...
6.  **Numeric or Text Input Slots (Card-like Inputs):** Are there prominent
    ...
    **`Text field`** (not just static `Text`). You MUST verify against the
...
8.  **Floating Action Buttons (FABs):** Is there a prominent button (circular or
    ...
    verify against specific M3 heuristics. Do not classify it as a `Chip` or standard button.
...
10. **Progress & Loading Indicators:** Is there a horizontal progress bar (often
    ...
    verify against specific heuristics. Do not ignore thin progress track lines or custom status tracks.
11. **Chips & Tags (Filter & Selection Controls):** Are there compact,
    ...
    yes, you MUST suspect them as **`Chip`** components. Do not ignore them or
...
14. **Thin Separating Lines (Dividers):** Are there thin horizontal or vertical
    ...
    Do not ignore them as decorative-only lines.
...
*   **Bottom Sheet (`Bottom sheet`):** A prominent surface anchored to the
    ...
    by rounded top corners (e.g. 28dp in MD3) and often a horizontal drag handle
    divider. You MUST explicitly map the outer container to **`Bottom sheet`**
    in addition to any inner lists, tabs, or buttons.
...
*   **Lists (`List` and `List item`):** When analyzing a vertical continuous
    ...
    *   *Individual Rows (`List item`):* Every distinct horizontal row within
        ...
        elevated or outlined background, map it as **`Card`**).
    *   *Contained Lists (List inside a Card container):* In modern settings
        screens, lists are often enclosed inside a unified, elevated or outlined
...
## Part 5: Global Theme Extraction

### 13. Page-Level Palette Extraction

*   **Palette:** Extract the page-level color scheme from the screenshot or stylesheets. Map the primary, secondary, background, and surface colors using their corresponding semantic M3 color roles.
    *   `primaryColor`
    *   `secondaryColor`
    *   `backgroundColor`
    *   `surfaceColor`
...
*   **Type Scale:** Identify the list of font families and weights used on the
    screen (e.g., `"Google Sans-Regular"`, `"Google Sans Text-Medium"`, `"Google Sans Flex"`) to populate
    `typeScale`.
