...
component mapping, styling attributes, and Google Material 3 (M3) design token
adherence in code.
...
> [!NOTE] **Material Web Components & Custom Elements are valid components.**
> UI code may use standard HTML/CSS, Web Components, gBreeze (`<md-gb-*>`),
> `@material/web` (`<md-*>`), or frontend framework libraries (such as
> Angular Material, MUI, Vuetify). Expect custom element tags such as
...
> as valid first-class components and map each to its canonical M3 component
> type (e.g., `<md-gb-button color="filled">` → `Filled button`).
...
style variables correspond properly to Google Material 3 (M3) design tokens:

### Custom Properties and Variables

-   Verify that colors are mapped to CSS variables (e.g., `var(--md-sys-color-primary)`
    or `var(--md-sys-color-surface)`) rather than using hardcoded raw hex strings.
...
-   Check that elevations are implemented using standard box shadows or M3
    elevation tokens (e.g., `var(--md-sys-elevation-level1)` through `var(--md-sys-elevation-level5)`).
...
-   Verify typography styles match designated values (e.g., 'Google Sans',
    'Google Sans Text', or specified sans-serif typography fallback).
...
-   `primaryColor`: Extract from `--md-sys-color-primary`, `--color-primary`,
...
If the theme variables are not mapped using `--md-sys-color-*` or standard
