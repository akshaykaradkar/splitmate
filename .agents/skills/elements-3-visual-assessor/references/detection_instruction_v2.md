# UI Detection & Segmentation Reference Guide for Elements GM3

This document provides the authoritative scanning rules required during the
`elements-3-visual-detector` Chain-of-Thought (CoT) iterative verification process. (For the
authoritative JSON output schema, consult [SKILL.md](../detector/SKILL.md)).

**CRITICAL PRINCIPLE - ELEMENTS GM3 ADHERENCE IS IRRELEVANT:** The sole purpose of
this detection phase is component identification. Whether a detected component
adheres to Elements GM3 Design System guidelines is completely irrelevant. Do not filter
out or misclassify components based on whether they follow Elements GM3 rules.
...
*   **Bottom Sheet (`Bottom sheet`):** A prominent surface anchored to the
    ...
    by rounded top corners (e.g. 28dp in standard GM3) and often a horizontal drag handle
...
*   **Snackbar (`Snackbar`):** A horizontal message container positioned at the
    ...
    brief message and optionally a action button (e.g., "Hide", "Accept",
...
*   **Palette:** Use the `color` skill to extract the page-level color scheme
    from the screenshot:
    ...
    keys) to populate the following fields in the `theme` object:
...
*   **Type Scale:** Identify the list of font families and weights used on the
    screen (e.g., `"Google Sans-Regular"`, `"Google Sans Text-Medium"`) to populate
    `typeScale`.
