---
name: elements-3-visual-detector
description: >-
...
extraction; whether a component is compliant with the Elements GM3 design system
is not relevant for this skill. It serves as the foundational detection step for
any Elements GM3 design audit.
...
3.  **No Exceptions:** You MUST NOT skip this step or rely on default model
    knowledge. The file access audit log must show that you read these guideline
    files in this turn of the conversation.
...
3.  **Elements GM3 Compliance Independence:** Remember that the sole objective of
    this skill is identifying components. Whether a component is Elements GM3
    compliant or not is completely irrelevant for this detection skill.

### Step 4: Global Theme Extraction

In addition to component identification, you MUST extract the overall page-level
theme to populate the `theme` object in the final JSON:

1.  **Palette:** Use the `color` skill to extract the page-level color scheme
    from the screenshot:

    ```bash
    blaze run //java/com/google/ux/gdp/ai/skills/color:color -- \
    ...
    object, ensuring they align with the design system bindings (e.g. `--cee3-sys-color-*`).
...
structural sub-element of a larger container. In Elements GM3 audits, every
...
### Step 6: Color / Layout Intent Placeholder

If the user's question pertains to color and/or layout that is not related to
specific components:

*   **[PLACEHOLDER: Non-Component Color & Layout Analysis]**
*   *Do nothing for now.* Output an empty or baseline structure indicating that
    non-component color/layout evaluation is pending future implementation.
