---
name: android-motion-visual-assessor
description: >-
  ...
  or perform a hybrid visual + codebase audit to evaluate Android Motion Design System
  compliance or answer specific design inquiries.
---

# Android Motion Visual Assessor

This skill is the authoritative standard for deep visual analysis, codebase
styling inspection, and Android Motion Design System adherence audits of UI
...
2.  **Codebase Evidence (Markup, Stylesheets & Tokens)**:
    -   *Markup & Components*: Inspect HTML, JSX, templates, or component source
        files (e.g., standard HTML elements, Android Motion `<droid-*>`, or frontend UI components).
    -   *Stylesheets*: Inspect CSS, SCSS, or style definitions for variables,
        elevation, padding, layout rules, and specifically animation/motion keyframes, durations, and easings.
    -   *Design Tokens*: Inspect design token specifications or theme
        definitions (e.g., `design_memory.md`, `DESIGN.md`, `tokens.json`, or
        theme CSS with `--droid-*` tokens).
...
> structure (DOM hierarchy, custom properties like `--droid-sys-*`, typography
> scale, and motion attributes) for maximum audit accuracy.
...
1.  **Read Detection Protocol References:**
    -   Read
        ...
        [code_detection_instruction.md](references/code_detection_instruction.md).
    -   Confirm visual tokens, color roles, shapes, and component rules in
        [carbon.md](references/carbon.md).
...
3.  **Code Inspection & Cross-Verification Loop (When Code Context Is
    Present):**
    ...
    -   Use code inspection to explicitly **confirm or challenge/refine** what
        ...
        standard `<droid-filled-button>`, confirming DOM hierarchy, or auditing CSS
        variables like `--droid-sys-color-primary` and motion definitions like `--droid-sys-motion-duration-200`).
...
our CSS variables mapped to Android Motion color and easing tokens?"*):
...
#### B. Comprehensive Android Motion Design Compliance Audits
...
1.  **Analysis & Evaluation (MANDATORY Two-Pass Model):**
    -   **Pass 1 (Foundational Screen-Level Evaluation):** Read
        ...
        across Typography, Color, Layout, Elevation, Motion, and Accessibility.
...
2.  **Audit & Reporting:**
    -   **Score Calculation & Rubric Completeness Verification:**
        -   **In ADK Environments (Function Calling):** If the
            `calculate_android_motion_score` (or corresponding scoring tool) is provided in your toolset, you **MUST**
            call it to verify rubric completeness and calculate compliance scores.
        -   **In CLI/Terminal Environments:** If command execution
            tools are available, execute the bundled scoring script:

            ```bash
            python3 ux/gdp/ai/design_systems/android-motion/skills/visual-assessor/scripts/calculate_score.py
            ```
        ...
        *   Issues actionable warnings if any questions were missed so you can
            backfill them.
        *   Deterministically computes the UI Hygiene, Android Motion Adherence,
