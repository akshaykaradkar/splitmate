...
This instruction defines how to calculate the overall Android Motion Design System
...
- **UI Hygiene (Usability & Polish):** Evaluates general visual polish, grid
  ...
  (`[Metric type: Hygiene]`).
- **Android Motion Adherence (System Specs):** Evaluates strict compliance with
  official Android Motion tokens, component geometry, typography families (Google
  Sans), color roles, shapes, and motion transitions (`[Metric type: Adherence]`).
...
- **Tier 0 (Brand Signature Anchors — Weight = 10):** Non-negotiable brand
  identity pillars, system anchor rules, and motion parameters (e.g., sentence case capitalization,
  surface color role separation, standard leading icons, active navigation
  pill containers, primary action reservation, strict motion token binding).
- **Tier 1 (Critical Component & Layout Rules — Weight = 5):** High-impact layout
  ...
  imagery, prominent actions, motion coherence).
- **Tier 2 (Key Android Motion Patterns & Component Rules — Weight = 3):** Standard
...
the Android Motion Adherence rate.
...
### Step 2: Android Motion Adherence Score (%)
$$\text{Android Motion Adherence Score (\%)} = \left(
...
\left( \frac{\text{Android Motion Adherence Score (\%)}}{100} \right)$$
...
- **85% – 100% (Production Ready / PASSED):** High UI polish combined with
  strict Android Motion adoption.
- **50% – 84% (In Transition / NEEDS_REFINEMENT):** Clean layout and good
  hygiene, but contains custom styling, legacy components, or missing brand/motion
  styles requiring remediation.
- **0% – 49% (Non-System / Custom App / NEEDS_REFINEMENT):** The application
...
`layoutScore`, `bottomSheetScore`, `buttonScore`, `auroraScore`, `motionScore`) are
...
| **`motionScore`** | `[Motion]` or `[Motion & Transitions]` | Foundational `rubric.md` |
