---
name: android-pixel-design-system-visual-assessor
description: >-
  ...
  or perform a hybrid visual + codebase audit to evaluate Android Pixel Design System
  compliance or answer specific design inquiries.
---

# Android Pixel Design System Visual Assessor

This skill is the authoritative standard for deep visual analysis, codebase
styling inspection, and Android Pixel Design System (APDS) adherence audits of UI
...
2.  **Codebase Evidence (Markup, Stylesheets & Tokens)**:
    ...
    -   *Markup & Components*: Inspect HTML, JSX, templates, or component source
        files (e.g., standard HTML elements, APDS custom elements, or frontend UI components).
...
> structure (DOM hierarchy, custom properties like `--apds-sys-*`, typography
...
3.  **Code Inspection & Cross-Verification Loop (When Code Context Is
    ...
    -   Use code inspection to explicitly **confirm or challenge/refine** what
        ...
        standard APDS button, confirming DOM hierarchy, or auditing CSS
        variables like `--apds-sys-color-primary`).
...
our CSS variables mapped to APDS color tokens?"*):
...
#### B. Comprehensive Android Pixel Design System Compliance Audits
...
2.  **Audit & Reporting:**
    ...
    -   **Score Calculation & Rubric Completeness Verification:**
        ...
        -   **In ADK Environments (Function Calling):** If the
            `calculate_apds_score` (or `calculate_m3_score`) tool is provided in your toolset, you **MUST**
            call `calculate_apds_score(rubric={...}, components=[...])` to verify
        ...
        -   **In CLI/Terminal Environments (e.g. Jetski):** If command execution
            ...
            ```bash
            python3 ux/gdp/ai/skills/android_pixel_design_system_visual_assessor/scripts/calculate_score.py
            ```
        ...
        *   Issues actionable warnings if any questions were missed so you can
            backfill them.
        *   Deterministically computes the UI Hygiene, APDS Adherence,
