---
name: elements-3-visual-assessor
description: >-
  ...
  or perform a hybrid visual + codebase audit to evaluate Elements GM3 design
  system compliance or answer specific design inquiries.
---

# Elements GM3 Visual Assessor

This skill is the authoritative standard for deep visual analysis, codebase
styling inspection, and Elements GM3 design system adherence audits of UI
...
> structure (DOM hierarchy, custom properties like `--cee3-sys-*` or `--md-sys-*`,
> typography scale) for maximum audit accuracy.
...
1.  **Read Detection Protocol References:**
    ...
    -   Confirm visual tokens, color roles, and component rules in
        [bindings.css](assets/design-system-bindings.css).
...
3.  **Code Inspection & Cross-Verification Loop (When Code Context Is
    ...
    -   Use code inspection to explicitly **confirm or challenge/refine** what
        ...
        standard `<md-gb-button>` or `<md-filled-button>`, confirming DOM hierarchy,
        or auditing CSS variables like `--cee3-sys-color-extended-blue` or
...
our CSS variables mapped to Elements GM3 color tokens?"*):
...
#### B. Comprehensive Elements GM3 Compliance Audits
...
2.  **Audit & Reporting:**
    ...
    -   **Score Calculation & Rubric Completeness Verification:**
        ...
        -   **In ADK Environments (Function Calling):** If the
            `calculate_elements3_score` or `calculate_m3_score` tool is provided in your toolset, you **MUST**
            call the appropriate function to verify rubric completeness and calculate
            the official compliance scores.
        -   **In CLI/Terminal Environments (e.g. Jetski):** If command execution
            tools are available, execute the bundled scoring script:

            ```bash
            python3 ux/gdp/ai/design_systems/elements-3/skills/visual-assessor/scripts/calculate_score.py
            ```
        ...
        *   Issues actionable warnings if any questions were missed so you can
            backfill them.
        *   Deterministically computes the UI Hygiene, Elements GM3 Adherence,
